...
describe("GroupFactory", () => {
  ...
  it("creates a group successfully", () => {
    ...
    const result = contract.createGroup("Alpha", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
  ...
  it("rejects duplicate group names", () => {
    contract.setAuthorityContract("ST2TEST");
    contract.createGroup("Alpha", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    const result = contract.createGroup("Alpha", 20, 200, 60, 10, 60, "urban", 15, 14, "CityY", "USD", 100, 2000);
  ...
  it("rejects non-authorized caller", () => {
    ...
    const result = contract.createGroup("Beta", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects group creation without authority contract", () => {
    const result = contract.createGroup("NoAuth", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid max members", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("InvalidMembers", 51, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid contribution amount", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("InvalidContrib", 10, 0, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid group type", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("InvalidType", 10, 100, 30, 5, 50, "invalid", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("updates a group successfully", () => {
    contract.setAuthorityContract("ST2TEST");
    contract.createGroup("OldGroup", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
  ...
  it("rejects update by non-creator", () => {
    contract.setAuthorityContract("ST2TEST");
    contract.createGroup("TestGroup", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
  ...
  it("returns correct group count", () => {
    contract.setAuthorityContract("ST2TEST");
    contract.createGroup("Group1", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    contract.createGroup("Group2", 15, 200, 60, 10, 60, "urban", 15, 14, "CityY", "USD", 100, 2000);
  ...
  it("rejects group creation with empty name", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects group creation with max groups exceeded", () => {
    ...
    contract.createGroup("Group1", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 1000);
    const result = contract.createGroup("Group2", 15, 200, 60, 10, 60, "urban", 15, 14, "CityY", "USD", 100, 2000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid interest rate", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("HighInterest", 10, 100, 30, 5, 50, "rural", 25, 7, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid grace period", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("BadGrace", 10, 100, 30, 5, 50, "rural", 10, 40, "VillageX", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid location", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("NoLocation", 10, 100, 30, 5, 50, "rural", 10, 7, "", "STX", 50, 1000);
    expect(result.ok).toBe(false);
  ...
  it("rejects invalid currency", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("BadCurrency", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "INVALID", 50, 1000);
    expect(result.ok).toBe(false);
    expect(result.value).toBe(ERR_INVALID_CURRENCY);
  });

  it("rejects invalid min contribution", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("BadMinContrib", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 0, 1000);
    expect(result.ok).toBe(false);
    expect(result.value).toBe(ERR_INVALID_MIN_CONTRIB);
  });

  it("rejects invalid max loan", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.createGroup("BadMaxLoan", 10, 100, 30, 5, 50, "rural", 10, 7, "VillageX", "STX", 50, 0);
    expect(result.ok).toBe(false);
    expect(result.value).toBe(ERR_INVALID_MAX_LOAN);
  });

  it("rejects setting authority contract twice", () => {
    contract.setAuthorityContract("ST2TEST");
    const result = contract.setAuthorityContract("ST3OTHER");
    expect(result.ok).toBe(false);
    expect(result.value).toBe(false);
  });

  it("rejects blackhole authority contract", () => {
